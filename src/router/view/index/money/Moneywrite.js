...
export default function Moneywrite() {
	const [showtf, setshowTf] = useState(false);
	const [info, setinfo] = useState([]);
	const {id} = useParams();
	useEffect(() => {
		request("/index/index/money_look", {id}).then((res) => {
			console.log(res);
			...
			{showtf&&info ? (
				<div>
					<Descriptions
						...
						title={`标题:${info.moeny_accessory}`}></Descriptions>
					<Descriptions
						style={{
							backgroundColor: "white",
							padding: "10px 20px",
					...
					<div dangerouslySetInnerHTML={{__html: info.money_info}} style={{backgroundColor: "white", padding: "10px 20px"}}></div>
				</div>
			) : (

				...
				if(type==1){
					obj[day] = (
						<div>
							...
							<div>{son.ke_name + "[" + son.start_time + "-" + son.end_time + "]"}</div>
							<div>{son.classroom}</div>
						</div>
					);
				}else {
					console.log(45545);
...
export default function Myclassday() {
	const [showtf, setshowTf] = useState(false);
	const [dataSource, setdataSource] = useState([]);
	const loc = JSON.parse(localStorage.getItem("user"));
	useEffect(() => {
		if (loc.user_id.length == 9 && loc.group == 0) {
			request("/teacher/index/my_class_day", {teacher_id: loc.user_id, nd: getNd()}).then(({data}) => {
				console.log(data);
				let arr = [];
